...
namespace Polenter.Serialization.Deserializing
{
    ...
    public sealed class ObjectFactory
    {
...
#if WINDOWS8
            // No idea what it is
            throw new InvalidOperationException(string.Format("Unknown Property type: {0}", property.GetType().GetTypeInfo().Name));  
#else
            // No idea what it is
...
#if WINDOWS8
            MethodInfo methodInfo = collection.GetType().GetTypeInfo().GetDeclaredMethod("Add");
#else



            MethodInfo methodInfo = collection.GetType().GetMethods().Where(method => method.Name == "Add" && method.GetParameters().Length == 1)
                .FirstOrDefault();
#endif
            if (methodInfo != null)
            {
...
#if WINDOWS8
            MethodInfo methodInfo = dictionary.GetType().GetTypeInfo().GetDeclaredMethod("Add");
#else
        ...
        private void fillProperties(object obj, IEnumerable<Property> properties)
        {
            ...
            {
#if WINDOWS8
                PropertyInfo propertyInfo = obj.GetType().GetTypeInfo().GetDeclaredProperty(property.Name);
#else
